...
parser.add_argument('--inputFile', dest='inputfile',
                    required=True, help="input yaml file to process")
    ...
    for item in inDictionary:
        ...
        if isinstance(item, dict):
            ...
            if isinstance(inDictionary, dict):
                ...
                for _item in inDictionary:
                    inPTable.add_row([indent + _item, SPACE_CHAR+SPACE_CHAR,
                                     SPACE_CHAR+SPACE_CHAR, listToString(get_sentences(1, True))])
                break
            ...
            if isinstance(moreStuff, dict):
                ...
                if indent == "":
                    inPTable.add_row(
                        [SPACE_CHAR, SPACE_CHAR, SPACE_CHAR, SPACE_CHAR])
                #
                inPTable.add_row(
                    [indent + item, "", "", listToString(get_sentences(1, True))])
                for dicInDic in moreStuff:
                    if dicInDic is not None:
                        if isinstance(dicInDic, dict):
                            printDic(dicInDic, inPTable, SPACE_CHAR +
                                     SPACE_CHAR + SPACE_CHAR + SPACE_CHAR + indent)
            else:
                # Most of the call will end-up eventually here -
                # this will print - key,value,isItRequired, Lorem ipsum (description)
                inPTable.add_row([indent + item, inDictionary[item], SPACE_CHAR +
                                 SPACE_CHAR, listToString(get_sentences(1, True))])
...
def generatePreviewFromYaml(INPUT_YAML):
    ...
    with open(INPUT_YAML) as file:
        ...
        for key in yaml_file_object:
            ...
            prettyTable.field_names = ["Attribute", "Value"]
            prettyTable.align["Attribute"] = "l"
            prettyTable.align["Value"] = "l"
            prettyTable.max_width["Attribute"] = 1
            prettyTable.max_width["Value"] = 3
            ...
            table = prettyTable.get_html_string(attributes={"name": key,
                                                            ...
                                                            "style": "width: 100%;table-layout: fixed;overflow-wrap: "
                                                            "break-word;font-size: 14px;margin-bottom: 5px;"})
