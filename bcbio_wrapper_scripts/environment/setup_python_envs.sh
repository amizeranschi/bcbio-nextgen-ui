...
if [[ ! -d ${bcbio_install_path:?}/extra ]]; then
   ...
   ${bcbio_install_path:?}/extra/bin/extra_conda install --yes -c conda-forge -c bioconda "mamba>=1"
   ...
   flye shasta repeatmodeler repeatmasker gxx_linux-64 wget git bedops vcftools pyyaml perl-yaml tectonic gnuplot "openjdk>=12" r-rjava \
   ...
   "ensembl-vep>=109" "perl-bioperl>=1.7.2" "r-base=4.3.1" r-essentials r-xml r-knitr r-markdown r-rmarkdown r-shiny r-shinyngs r-rcpp r-devtools r-reticulate \
