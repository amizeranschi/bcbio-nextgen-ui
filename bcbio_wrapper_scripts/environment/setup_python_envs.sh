...
if [[ ! -d ${bcbio_install_path}/extra ]]; then
   ...
   ${bcbio_install_path}/extra/bin/mamba_extra create --name py3 python=3.9 -y
   ${bcbio_install_path}/extra/bin/mamba_extra create --name py2 python=2.7.18 -y
   
   echo "--- [$(date +"%F %R")] Installing packages in the Python2 environment"
   
   ${bcbio_install_path}/extra/bin/mamba_extra install --name py2 --yes -c conda-forge -c bioconda faststructure pathoscope
   ...
   ${bcbio_install_path}/extra/bin/mamba_extra install --name py3 --yes -c conda-forge -c bioconda kraken2 blast spades masurca quast \
   gxx_linux-64 wget git bedops vcftools pyyaml perl-yaml \
   ...
   "ensembl-vep>=105" "perl-bioperl>=1.7.2" "r-base>=4.2" r-essentials r-xml r-knitr r-markdown r-rmarkdown r-shiny r-rcpp r-devtools r-reticulate \
